...
numbers_list = [int(i) for i in input_numbers.split()]
numbers_tuple = tuple((int(i) for i in input_numbers.split()))
numbers_set = set((int(i) for i in input_numbers.split()))

set2 = {2, 4, 55, 17, 8, 10}

numbers_dict = {int(i): int(i) ** 2 for i in input_numbers.split()}
...
# Difference
    set_difference = numbers_set.difference(set2)
...
list_to_dict = {val: val ** 2 for val in numbers_list}
tuple_to_list = list(numbers_tuple)
tuple_to_set = set(numbers_tuple)
tuple_to_dict = {val: val ** 2 for val in numbers_tuple}
set_to_list = list(numbers_set)
set_to_tuple = tuple(numbers_set)
set_to_dict = {val: val ** 2 for val in numbers_set}
...
num_integers = 0
for line in lines:
    integers = re.findall(r'[-+]?\d*\.\d+|\d+', line)
    integers = [int(num) for num in integers if num.strip('-').isdigit()]
...
print("Sum of all integers in the file:", sum_integers)
...
def find_largest_integer(fname):
    integers_set = set()
    with open(fname, 'r') as file:
        for line in file:
            integers = re.findall(r'[-+]?\d+', line)
            integers_set.update(map(int, integers))
    return list(integers_set)[-1]


fname1 = 'output.txt'
fname2 = 'prime_numbers.txt'

max_int = find_largest_integer(fname1)
if max_int is None:
    print("Error: Largest integer not found in the file.")
else:
    print("Largest integer: " + str(max_int))

    #Generate a list of all prime numbers up to the largest integer.
    primes = [num for num in range(2, max_int + 1) if is_prime(num)]
    print("List of prime numbers: " + ', '.join(map(str, primes)))

    #Calculate the sum of all prime numbers in the list.
    prime_sum = sum(primes)
    print("Sum of prime numbers: " + str(prime_sum))

    #Determine the largest and smallest prime numbers in the list.
    max_prime = max(primes)
    print("Largest prime number: " + str(max_prime))

    min_prime = min(primes)
    print("Smallest prime number: " + str(min_prime))

    #Check if the largest integer itself is prime and print the result.
    largest_is_prime = is_prime(max_int)
    print("The largest integer" + (" is" if largest_is_prime else " is not") + " prime")
    ...
    with open(fname2, 'w') as file:
        file.write("List of prime numbers: " + ', '.join(map(str, primes)) + "\n")
        file.write("Sum of prime numbers: " + str(prime_sum) + "\n")
        file.write("Largest prime number: " + str(max_prime) + "\n")
        file.write("Smallest prime number: " + str(min_prime) + "\n")
        file.write("The largest integer" + (" is" if largest_is_prime else " is not") + " prime\n")
